<template>
  <div class="mx-auto w-full max-w-3xl text-center">
    ...
    <div class="mb-8 transition-all delay-300 duration-150 ease-in-out">
      <div ref="testimonialsRef" class="relative flex flex-col">
        <transition-group name="testimonial-text">
          ...
          >
            <div
              class="text-2xl font-bold text-zinc-900 before:content-['\201C'] after:content-['\201D']"
            >
              {{ testimonial.quote }}
            </div>
          </div>
    ...
    <div
      class="mt-4 flex w-full items-center justify-between gap-4 pt-12 md:pt-0"
    >
      <button
...
function startAutorotate() {
  intervalId = window.setInterval(() => {
    active.value =
      active.value + 1 === props.testimonials.length ? 0 : active.value + 1;
    heightFix();
  }, props.duration * 1000);
}
